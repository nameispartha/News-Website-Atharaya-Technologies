...
import { getById, update } from '../../services/newsService'
...
export default function EditNews() {
  ...
  const [selectedSubcategory, setSelectedSubcategory] = useState('Hollywood')
    ...
    <div className="space-y-6">
      ...
      >
        <div className="col-span-1 md:col-span-2">
          ...
          <input name="title" defaultValue={getById(id)?.title || `News #${id}`} type="text" className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-500" />
        </div>
        <div>
        ...
        <div>
          ...
          <input name="author" type="text" defaultValue={getById(id)?.author || 'Author'} className="w-full border rounded-lg px-3 py-2" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Date</label>
          <input name="date" type="date" defaultValue={getById(id)?.date} className="w-full border rounded-lg px-3 py-2" />
        </div>
        <div className="col-span-1 md:col-span-2">
          <label className="block text-sm font-medium mb-1">Content</label>
          <textarea name="content" rows="8" className="w-full border rounded-lg px-3 py-2">{getById(id)?.content || 'Existing content...'}</textarea>
        </div>
        <div className="col-span-1 md:col-span-2 flex items-center justify-end gap-2">
          ...
          <button type="submit" className="px-4 py-2 rounded bg-yellow-500 text-black font-semibold hover:bg-yellow-400">Update</button>
        </div>
      </form>
    </div>
  )
